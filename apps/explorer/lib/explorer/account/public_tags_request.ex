defmodule Explorer.Account.PublicTagsRequest do
  ...
  @attrs ~w(company website description remove_reason request_id status)a
  ...
  def public_tags_requests_by_identity_id_query(id) when not is_nil(id) do
    __MODULE__
    |> where(
      [request], request.identity_id == ^id and request.request_type != "delete"
    )
  ...
  def update(%{id: id, identity_id: identity_id} = attrs) do
    with public_tags_request <- get_public_tags_request_by_id_and_identity_id(id, identity_id),
         false <- is_nil(public_tags_request) do
        #  {:ok, changeset} <-
           public_tags_request |> changeset(Map.put(attrs, :request_type, "edit")) |> Repo.account_repo().update()
          #  do
