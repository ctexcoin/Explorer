defmodule Explorer.Tags.AddressToTag do
  ...
  * `:address_hash` - hash of Address
  ...
  def set_addresses_to_tag(tag_id, address_hash_string) do
    with nil <- get_addresses_to_tag_id(tag_id,address_hash_string),
          :ok <- Chain.check_address_exists(address_hash_string) do
        #Logger.info("--- set_addresses_to_tag -------------#{tag_id} ---------- #{address_hash_string} ----------- ")
        %AddressToTag{}
        |> AddressToTag.changeset(%{address_hash: address_hash_string, tag_id: tag_id})
        |> Repo.insert()
    else
      _ ->
        nil
  ...
  def get_addresses_to_tag_id(nil), do: nil

  def get_addresses_to_tag_id(tag_id, address_hash_string) do
    query =
      from(
        tag in AddressToTag,
        where: tag.tag_id == ^tag_id,
        where: tag.address_hash == ^address_hash_string,
        select: tag.id
      )
  ...
  def delete_addresses_to_tag(tag_id, address_hash_string) do
    with tag <- get_addresses_to_tag_id(tag_id,address_hash_string),
        false <- is_nil(tag) do
        # Logger.info("--- set_addresses_to_tag -------------#{tag} ---------- #{address_hash_string} ----------- ")
        delete_query =
          from(
            att in AddressToTag,
            where: att.id == ^tag
