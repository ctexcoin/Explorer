defmodule BlockScoutWeb.Account.TagAddressController do
  ...
  def index(conn, _params) do
    ...
    render(conn, "index.html", address_tags: TagAddress.get_tags_address_by_identity_id(current_user.id), user: current_user)
  end

  def new(conn, _params) do
    current_user = authenticate!(conn)

    render(conn, "form.html", tag_address: new_tag(), user: current_user)
  end
  ...
  def create(conn, %{"tag_address" => tag_address}) do
    ...
    case TagAddress.create(%{
      ...
      {:error, invalid_tag_address} ->
        render(conn, "form.html", tag_address: invalid_tag_address, user: current_user)
    end
  end
