defmodule BlockScoutWeb.Account.TagTransactionController do
  ...
  def index(conn, _params) do
    ...
    render(conn, "index.html", tx_tags: TagTransaction.get_tags_transaction_by_identity_id(current_user.id), user: current_user)
  end

  def new(conn, _params) do
    current_user = authenticate!(conn)

    render(conn, "form.html", tag_transaction: new_tag(), user: current_user)
  end
  ...
  def create(conn, %{"tag_transaction" => tag_address}) do
    ...
    case TagTransaction.create(%{
      ...
      {:error, invalid_tag_transaction} ->
        render(conn, "form.html", tag_transaction: invalid_tag_transaction, user: current_user)
    end
  end
