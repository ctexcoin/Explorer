defmodule BlockScoutWeb.Account.ApiKeyController do
  ...
  def new(conn, _params) do
    ...
    render(conn, "form.html", method: :create, api_key: empty_api_key(), user: current_user)
  end
  ...
  def create(conn, %{"key" => api_key}) do
    ...
    case ApiKey.create(%{name: api_key["name"], identity_id: current_user.id}) do
      ...
      {:error, invalid_api_key} ->
        render(conn, "form.html", method: :create, api_key: invalid_api_key, user: current_user)
    end
  end
  ...
  def index(conn, _params) do
    ...
    render(conn, "index.html", api_keys: ApiKey.get_api_keys_by_identity_id(current_user.id), user: current_user)
  end
  ...
  def edit(conn, %{"id" => api_key}) do
    ...
    case ApiKey.get_api_key_by_value_and_identity_id(api_key, current_user.id) do
      ...
      %ApiKey{} = api_key ->
        render(conn, "form.html", method: :update, api_key: ApiKey.changeset(api_key), user: current_user)
    end
  end
