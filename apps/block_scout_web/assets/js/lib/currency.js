...
function formatPercentChangeValue (value) {
  if (isNaN(value)) return '(+0.00%)'
  const prefix = value > 0 ? '+' : ''
  if (value === 0 || value === '0') return `(${prefix}0.00%)`
  if (value < 1) return `(${prefix}${numeral(value).format('0.00')}%)`
  if (value < 100000) return `(${prefix}${numeral(value).format('0,0.00')}%)`
...
function tryUpdatePercentChange24hValues (el, percentChange24h = el.dataset.pricePercentChange) {
  const formattedValue = formatPercentChangeValue(percentChange24h)
  if (formattedValue !== el.innerHTML) el.innerHTML = formattedValue
}
...
export function updateAllPercentChange24hValues (percentChange24h) {
  $('[data-price-percent-change]').each((i, el) => tryUpdatePercentChange24hValues(el, percentChange24h))
}
updateAllCalculatedUsdValues()
updateAllPercentChange24hValues();
